...
module.exports = function(logger){
    ...
    var buildKeyScriptPage = function(){
        async.waterfall([
            ...
            function(client, coinBytes, missingCoins, callback){
                ...
                async.each(missingCoins, function(c, cback){
                    ...
                    daemon.cmd('dumpprivkey', [coinInfo.address], function(result){
                        if (result[0].error){
                            logger.error(logSystem, c, 'Could not dumpprivkey for ' + c + ' ' + JSON.stringify(result[0].error) + 'address ' + coinInfo.address);
                            cback();
                            return;
                        }
			...
			logger.error('pubkey : ' + vBytePub.toString() + ', ' + 'privKey : ' + vBytePriv.toString());
