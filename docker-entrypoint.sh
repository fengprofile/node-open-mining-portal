...
if [ "$1" == node ]; then
        ...
        : "${REDIS_PORT:=6379}"
        : "${COIND_HOST:=192.168.0.1}"
        : "${COIND_PORT:=8398}"
        : "${COIND_USER:=rpc}"
        : "${COIND_PASSWORD:=g6eJ6ZLCiws7mc0SVwlWhI7h4ve5yH}"
        : "${COIND_P2P_ENABLED:=false}"
        : "${COIND_P2P_HOST:=192.168.0.1}"
        : "${COIND_P2P_PORT:=8318}"
...
EOJS
        ...
        set_config "pool_configs/dash.json" "address" "Tc9tamXfxTN7dDEbYeSTVFgHkuy6pGmfAC"	string
#       set_config "pool_configs/dash.json" "rewardRecipients.TcvqNVtW7WzH6GwMejVgv6aGsRrU27iLc8" "1.0"	float    
